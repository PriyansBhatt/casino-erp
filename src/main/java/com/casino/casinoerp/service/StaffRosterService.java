...
public class StaffRosterService {
    ...
    private final ShiftDefinitionRepository shifts;private final UserRepository users;private final AuthenticatedUserService authenticatedUsers;
    ...
    public StaffRosterService(StaffRosterAssignmentRepository rosters,StaffProfileRepository profiles,ShiftDefinitionRepository shifts,
            UserRepository users,AuthenticatedUserService authenticatedUsers,CurrentUserRoleService currentRoles,RolePermissionService permissions,AuditLogService audit){this.rosters=rosters;this.profiles=profiles;this.shifts=shifts;this.users=users;this.authenticatedUsers=authenticatedUsers;this.currentRoles=currentRoles;this.permissions=permissions;this.audit=audit;}
    ...
    @Transactional public StaffRosterResponse create(CreateStaffRosterRequest request){
        requireManager();StaffProfile staff=eligibleStaff(request.staffProfileId());ShiftDefinition shift=activeShift(request.shiftDefinitionId());validateNoOverlap(null,staff.getId(),request.rosterDate(),shift);
    ...
    @Transactional public StaffRosterResponse update(UUID id,UpdateStaffRosterRequest request){
        requireManager();StaffRosterAssignment value=required(id);if(value.getStatus()!=RosterStatus.SCHEDULED)throw new ResourceConflictException("Only a SCHEDULED roster assignment can be updated.");StaffProfile staff=eligibleStaff(value.getStaffProfileId());ShiftDefinition shift=activeShift(request.shiftDefinitionId());validateNoOverlap(id,staff.getId(),request.rosterDate(),shift);
    ...
    private StaffProfile eligibleStaff(UUID id){StaffProfile v=profiles.findById(id).orElseThrow(()->new ResourceNotFoundException("Staff Profile not found."));if(v.getEmploymentStatus()!=EmploymentStatus.ACTIVE)throw new IllegalArgumentException("Only an ACTIVE Staff Profile can be scheduled.");return v;}
