...
class HotelBookingSecurityTests {
    ...
    @ParameterizedTest @ValueSource(strings={"DIRECTOR","SUPER_ADMIN"}) void managementAllowed(String role)throws Exception{mvc.perform(post("/api/hotel-bookings").with(user(role).roles(role)).contentType("application/json").content(REQUEST)).andExpect((role.equals("SUPER_ADMIN")?status().isCreated():status().isForbidden()));mvc.perform(get("/api/hotel-bookings/current").with(user(role).roles(role))).andExpect(status().isOk());}
    ...
    @Test void invalidGuestCountAndNegativeCostRejected()throws Exception{mvc.perform(post("/api/hotel-bookings").with(user("admin").roles("SUPER_ADMIN")).contentType("application/json").content(REQUEST.replace("\"numberOfGuests\":1","\"numberOfGuests\":0"))).andExpect(status().isBadRequest());mvc.perform(post("/api/hotel-bookings").with(user("admin").roles("SUPER_ADMIN")).contentType("application/json").content(REQUEST.replace("\"estimatedCost\":0","\"estimatedCost\":-1"))).andExpect(status().isBadRequest());}
}
