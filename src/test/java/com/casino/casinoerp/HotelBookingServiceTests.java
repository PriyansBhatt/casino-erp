...
class HotelBookingServiceTests {
    ...
    private final HotelBookingService service=new HotelBookingService(bookings,customers,sessions,dates,lock,auth,roles,new RolePermissionService(),users,audit);
    ...
    @Test void unauthorizedRoleDenied(){when(roles.getCurrentRole()).thenReturn(Optional.of(Role.CASHIER));assertThatThrownBy(()->service.create(request())).hasMessageContaining("restricted to Director or Super Admin");}
    @Test void requestedBookingCanBeApproved(){HotelBooking value=existing(HotelBookingStatus.REQUESTED);when(bookings.findById(value.getId())).thenReturn(Optional.of(value));assertThat(service.approve(value.getId()).status()).isEqualTo(HotelBookingStatus.APPROVED);assertThat(value.getApprovedBy()).isEqualTo(actorId);verify(audit).log(eq("APPROVE_HOTEL_BOOKING"),eq("HOTEL_BOOKING"),eq(value.getId()),eq(actorId),contains("APPROVED"));}
    @Test void requestedBookingCanBeRejected(){HotelBooking value=existing(HotelBookingStatus.REQUESTED);when(bookings.findById(value.getId())).thenReturn(Optional.of(value));assertThat(service.reject(value.getId()).status()).isEqualTo(HotelBookingStatus.REJECTED);}
    @Test void currentListUsesOpenBusinessDateAndRepositoryReload(){BusinessDate open=new BusinessDate();open.setBusinessDate(businessDate);when(dates.getCurrentOpenBusinessDate()).thenReturn(Optional.of(open));when(bookings.findByBusinessDateOrderByCreatedAtDesc(businessDate)).thenReturn(List.of());when(customers.findAllById(any())).thenReturn(List.of());when(sessions.findAllById(any())).thenReturn(List.of());assertThat(service.current()).isEmpty();verify(bookings).findByBusinessDateOrderByCreatedAtDesc(businessDate);}
